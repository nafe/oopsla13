...
// ---------------------------------------------------------------------------
#define ilog2(x) \
  (__ite_dtype(x == 1, 0, (__ite_dtype((2 <= x) & (x < 4), 1, 2))))

#define term(x,i) \
  (x - (__ite_dtype((0 <= i) & isone(0, (x+1)), 1, 0) + __ite_dtype((1 <= i) & isone(1, (x+1)), 2, 0)))

#define downsweep_summation(result,ghostsum,x) \
  (result[x] == (raddf(raddf(__ite_rtype(isone(1,(x+1)) & (1 < ilog2(x+1)), ghostsum[term(x,1)], 0),__ite_rtype(isone(0,(x+1)) & (0 < ilog2(x+1)), ghostsum[term(x,0)], 0)),ghostsum[x])))

#define downsweep_core(offset,result,ghostsum,x) \
  (__implies(((offset == 8) | \
            (!updated(x,2) & ((offset == 4) | \
            (!updated(x,1) & ((offset == 2)))))), (result[x] == ghostsum[x])) & \
   __implies(((offset <= 4) & updated(x,2)) | ((offset <= 2) & updated(x,1)), downsweep_summation(result,ghostsum,x)))

#if defined(INC_ENDSPEC) && defined(BINOP_ADD)
#define downsweep_nooverflow(offset,result,ghostsum,x) \
  __implies(((offset <= 4) & updated(x,2)) | ((offset <= 2) & updated(x,1)), __add_noovfl(__ite_rtype(isone(1,(x+1)) & (1 < ilog2(x+1)), ghostsum[term(x,1)], 0), __ite_rtype(isone(0,(x+1)) & (0 < ilog2(x+1)), ghostsum[term(x,0)], 0), ghostsum[x]))
...
#define downsweep_barrier(tid,offset,result,ghostsum) \
  (__implies((tid < 4) & (offset > 2), downsweep(offset,result,ghostsum,ai_idx(1,tid))) & \
   __implies((tid < 2) & (offset > 4), downsweep(offset,result,ghostsum,ai_idx(2,tid))) & \
   __implies((tid < 1),                downsweep(offset,result,ghostsum,ai_idx(div2(offset),tid))) & \
   __implies((tid < 1),                downsweep(offset,result,ghostsum,bi_idx(4,tid))) & \
   __implies((tid < 1) & (offset == 4), downsweep(offset,result,ghostsum,lf_ai_idx(4,tid))) & \
   __implies((tid < 3) & (offset == 2), downsweep(offset,result,ghostsum,lf_ai_idx(2,tid))) & \
   __implies((tid < 1) & (offset == 4), downsweep(offset,result,ghostsum,lf_bi_idx(4,tid))) & \
   __implies((tid < 3) & (offset == 2), downsweep(offset,result,ghostsum,lf_bi_idx(2,tid))))

#define downsweep_d_offset \
  (((d == 2) & (offset == 8)) | ((d == 4) & (offset == 4)) | ((d == 8) & (offset == 2)))
...
#define downsweep_barrier_permissions(tid,offset,result,ghostsum) \
  {if ((tid < 4) & (offset > 2)) downsweep_permissions(offset,result,ghostsum,ai_idx(1,tid)) \
   if ((tid < 2) & (offset > 4)) downsweep_permissions(offset,result,ghostsum,ai_idx(2,tid)) \
   if (tid < 1)                   downsweep_permissions(offset,result,ghostsum,ai_idx(div2(offset),tid)) \
   if (tid < 1)                   downsweep_permissions(offset,result,ghostsum,bi_idx(4,tid)) \
   if ((tid < 1) & (offset == 4)) downsweep_permissions(offset,result,ghostsum,lf_ai_idx(4,tid)) \
   if ((tid < 3) & (offset == 2)) downsweep_permissions(offset,result,ghostsum,lf_ai_idx(2,tid)) \
   if ((tid < 1) & (offset == 4)) downsweep_permissions(offset,result,ghostsum,lf_bi_idx(4,tid)) \
   if ((tid < 3) & (offset == 2)) downsweep_permissions(offset,result,ghostsum,lf_bi_idx(2,tid))}
...
#define final_downsweep_barrier(tid,result,ghostsum) \
  (__implies((tid < 1), downsweep(/*offset=*/2,result,ghostsum,ai_idx(1,tid))) & \
   __implies((tid < 1), downsweep(/*offset=*/2,result,ghostsum,bi_idx(4,tid))) & \
   __implies((tid < 3), downsweep(/*offset=*/2,result,ghostsum,lf_ai_idx(2,tid))) & \
   __implies((tid < 3), downsweep(/*offset=*/2,result,ghostsum,lf_bi_idx(2,tid))))

#if defined(SPEC_THREADWISE)
#define upsweep_instantiation \
  tid, (tid+1), div2((tid+1)), other_tid, (other_tid+1), div2((other_tid+1))
...
#define upsweep_instantiation \
  x2t(tid), (x2t(tid)+1), div2((x2t(tid)+1)), x2t(other_tid), (x2t(other_tid)+1), div2((x2t(other_tid)+1))
