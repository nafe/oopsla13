...
// ---------------------------------------------------------------------------
#if N != 32
  #error This specification is only valid when N=32
#endif
...
#define iselement(x)  ((0 < x) & (x < 32))
#define isthreadid(t) ((0 < t) & (t < 16))
...
#define upsweep_barrier(tid,offset,result,len) \
  (__implies((tid < 16) & (offset >= 1), upsweep(offset,result,len,ai_idx(1,tid))) & \
  __implies((tid < 8) & (offset >= 4), upsweep(offset,result,len,ai_idx(2,tid))) & \
  __implies((tid < 4) & (offset >= 8), upsweep(offset,result,len,ai_idx(4,tid))) & \
  __implies((tid < 2) & (offset >= 16), upsweep(offset,result,len,ai_idx(8,tid))) & \
  __implies((tid < 1) & (offset >= 32), upsweep(offset,result,len,ai_idx(16,tid))) & \
  __implies((tid < 16) & (offset <= 2), upsweep(offset,result,len,bi_idx(1,tid))) & \
  __implies((tid < 8) & (offset == 4), upsweep(offset,result,len,bi_idx(2,tid))) & \
  __implies((tid < 4) & (offset == 8), upsweep(offset,result,len,bi_idx(4,tid))) & \
  __implies((tid < 2) & (offset == 16), upsweep(offset,result,len,bi_idx(8,tid))) & \
  __implies((tid < 1) & (offset == 32), upsweep(offset,result,len,bi_idx(16,tid))))

#define upsweep_d_offset \
  ((d == 16 & offset == 1) | (d == 8 & offset == 2) | (d == 4 & offset == 4) | (d == 2 & offset == 8) | (d == 1 & offset == 16) | (d == 0 & offset == 32))

#define upsweep_permissions(offset,result,len,x) \
  {ghostread2 = ghostread4 = ghostread8 = ghostread16 = ghostread32 = false; \
...
#define upsweep_barrier_permissions(tid,offset,result,len) \
  {bool ghostread2, ghostread4, ghostread8, ghostread16, ghostread32; \
  if ((tid < 16) && (offset >= 1)) upsweep_permissions(offset,result,len,ai_idx(1,tid)) \
  if ((tid < 8) && (offset >= 4)) upsweep_permissions(offset,result,len,ai_idx(2,tid)) \
  if ((tid < 4) && (offset >= 8)) upsweep_permissions(offset,result,len,ai_idx(4,tid)) \
  if ((tid < 2) && (offset >= 16)) upsweep_permissions(offset,result,len,ai_idx(8,tid)) \
  if ((tid < 1) && (offset >= 32)) upsweep_permissions(offset,result,len,ai_idx(16,tid)) \
  if ((tid < 16) && (offset <= 2)) upsweep_permissions(offset,result,len,bi_idx(1,tid)) \
  if ((tid < 8) && (offset == 4)) upsweep_permissions(offset,result,len,bi_idx(2,tid)) \
  if ((tid < 4) && (offset == 8)) upsweep_permissions(offset,result,len,bi_idx(4,tid)) \
  if ((tid < 2) && (offset == 16)) upsweep_permissions(offset,result,len,bi_idx(8,tid)) \
  if ((tid < 1) && (offset == 32)) upsweep_permissions(offset,result,len,bi_idx(16,tid))}

// ---------------------------------------------------------------------------
// DOWNSWEEP INVARIANTS
// ---------------------------------------------------------------------------
#define ilog2(x) \
  (__ite_dtype(x == 1, 0, (__ite_dtype((2 <= x) & (x < 4), 1, (__ite_dtype((4 <= x) & (x < 8), 2, (__ite_dtype((8 <= x) & (x < 16), 3, 4))))))))

#define term(x,i) \
  (x - (__ite_dtype((0 <= i) & isone(0, (x+1)), 1, 0) + __ite_dtype((1 <= i) & isone(1, (x+1)), 2, 0) + __ite_dtype((2 <= i) & isone(2, (x+1)), 4, 0) + __ite_dtype((3 <= i) & isone(3, (x+1)), 8, 0)))

#define downsweep_summation(result,ghostsum,x) \
  (result[x] == (raddf(raddf(raddf(raddf(__ite_rtype(isone(3,(x+1)) & (3 < ilog2(x+1)), ghostsum[term(x,3)], 0),__ite_rtype(isone(2,(x+1)) & (2 < ilog2(x+1)), ghostsum[term(x,2)], 0)),__ite_rtype(isone(1,(x+1)) & (1 < ilog2(x+1)), ghostsum[term(x,1)], 0)),__ite_rtype(isone(0,(x+1)) & (0 < ilog2(x+1)), ghostsum[term(x,0)], 0)),ghostsum[x])))
...
#define downsweep_core(offset,result,ghostsum,x) \
  (__implies(((offset == 32) | \
   ...
   __implies(((offset <= 16) & updated(x,8)) | ((offset <= 8) & updated(x,4)) | ((offset <= 4) & updated(x,2)) | ((offset <= 2) & updated(x,1)), downsweep_summation(result,ghostsum,x)))

#if defined(INC_ENDSPEC) && defined(BINOP_ADD)
#define downsweep_nooverflow(offset,result,ghostsum,x) \
  __implies(((offset <= 16) & updated(x,8)) | ((offset <= 8) & updated(x,4)) | ((offset <= 4) & updated(x,2)) | ((offset <= 2) & updated(x,1)), __add_noovfl(__ite_rtype(isone(3,(x+1)) & (3 < ilog2(x+1)), ghostsum[term(x,3)], 0), __ite_rtype(isone(2,(x+1)) & (2 < ilog2(x+1)), ghostsum[term(x,2)], 0), __ite_rtype(isone(1,(x+1)) & (1 < ilog2(x+1)), ghostsum[term(x,1)], 0), __ite_rtype(isone(0,(x+1)) & (0 < ilog2(x+1)), ghostsum[term(x,0)], 0), ghostsum[x]))
...
#define downsweep_barrier(tid,offset,result,ghostsum) \
  (__implies((tid < 16) & (offset > 2), downsweep(offset,result,ghostsum,ai_idx(1,tid))) & \
   __implies((tid < 8) & (offset > 4), downsweep(offset,result,ghostsum,ai_idx(2,tid))) & \
   __implies((tid < 4) & (offset > 8), downsweep(offset,result,ghostsum,ai_idx(4,tid))) & \
   __implies((tid < 2) & (offset > 16), downsweep(offset,result,ghostsum,ai_idx(8,tid))) & \
   __implies((tid < 1),                downsweep(offset,result,ghostsum,ai_idx(div2(offset),tid))) & \
   __implies((tid < 1),                downsweep(offset,result,ghostsum,bi_idx(16,tid))) & \
   __implies((tid < 1) & (offset == 16), downsweep(offset,result,ghostsum,lf_ai_idx(16,tid))) & \
   __implies((tid < 3) & (offset == 8), downsweep(offset,result,ghostsum,lf_ai_idx(8,tid))) & \
   __implies((tid < 7) & (offset == 4), downsweep(offset,result,ghostsum,lf_ai_idx(4,tid))) & \
   __implies((tid < 15) & (offset == 2), downsweep(offset,result,ghostsum,lf_ai_idx(2,tid))) & \
   __implies((tid < 1) & (offset == 16), downsweep(offset,result,ghostsum,lf_bi_idx(16,tid))) & \
   __implies((tid < 3) & (offset == 8), downsweep(offset,result,ghostsum,lf_bi_idx(8,tid))) & \
   __implies((tid < 7) & (offset == 4), downsweep(offset,result,ghostsum,lf_bi_idx(4,tid))) & \
   __implies((tid < 15) & (offset == 2), downsweep(offset,result,ghostsum,lf_bi_idx(2,tid))))

#define downsweep_d_offset \
  (((d == 2) & (offset == 32)) | ((d == 4) & (offset == 16)) | ((d == 8) & (offset == 8)) | ((d == 16) & (offset == 4)) | ((d == 32) & (offset == 2)))
...
#define downsweep_barrier_permissions(tid,offset,result,ghostsum) \
  {if ((tid < 16) & (offset > 2)) downsweep_permissions(offset,result,ghostsum,ai_idx(1,tid)) \
   if ((tid < 8) & (offset > 4)) downsweep_permissions(offset,result,ghostsum,ai_idx(2,tid)) \
   if ((tid < 4) & (offset > 8)) downsweep_permissions(offset,result,ghostsum,ai_idx(4,tid)) \
   if ((tid < 2) & (offset > 16)) downsweep_permissions(offset,result,ghostsum,ai_idx(8,tid)) \
   if (tid < 1)                   downsweep_permissions(offset,result,ghostsum,ai_idx(div2(offset),tid)) \
   if (tid < 1)                   downsweep_permissions(offset,result,ghostsum,bi_idx(16,tid)) \
   if ((tid < 1) & (offset == 16)) downsweep_permissions(offset,result,ghostsum,lf_ai_idx(16,tid)) \
   if ((tid < 3) & (offset == 8)) downsweep_permissions(offset,result,ghostsum,lf_ai_idx(8,tid)) \
   if ((tid < 7) & (offset == 4)) downsweep_permissions(offset,result,ghostsum,lf_ai_idx(4,tid)) \
   if ((tid < 15) & (offset == 2)) downsweep_permissions(offset,result,ghostsum,lf_ai_idx(2,tid)) \
   if ((tid < 1) & (offset == 16)) downsweep_permissions(offset,result,ghostsum,lf_bi_idx(16,tid)) \
   if ((tid < 3) & (offset == 8)) downsweep_permissions(offset,result,ghostsum,lf_bi_idx(8,tid)) \
   if ((tid < 7) & (offset == 4)) downsweep_permissions(offset,result,ghostsum,lf_bi_idx(4,tid)) \
   if ((tid < 15) & (offset == 2)) downsweep_permissions(offset,result,ghostsum,lf_bi_idx(2,tid))}
...
#define final_upsweep_barrier(tid,result,len) \
  (__implies((tid < 16), upsweep(/*offset=*/N,result,len,ai_idx(1,tid))) & \
   __implies((tid < 8), upsweep(/*offset=*/N,result,len,ai_idx(2,tid))) & \
   __implies((tid < 4), upsweep(/*offset=*/N,result,len,ai_idx(4,tid))) & \
   __implies((tid < 2), upsweep(/*offset=*/N,result,len,ai_idx(8,tid))) & \
   __implies((tid < 1), upsweep(/*offset=*/N,result,len,ai_idx(16,tid))) & \
   __implies((tid < 1), upsweep(/*offset=*/N,result,len,bi_idx(16,tid))))

#define final_downsweep_barrier(tid,result,ghostsum) \
  (__implies((tid < 1), downsweep(/*offset=*/2,result,ghostsum,ai_idx(1,tid))) & \
   __implies((tid < 1), downsweep(/*offset=*/2,result,ghostsum,bi_idx(16,tid))) & \
   __implies((tid < 15), downsweep(/*offset=*/2,result,ghostsum,lf_ai_idx(2,tid))) & \
   __implies((tid < 15), downsweep(/*offset=*/2,result,ghostsum,lf_bi_idx(2,tid))))

#if defined(SPEC_THREADWISE)
#define upsweep_instantiation \
  tid, (tid+1), div2((tid+1)), div2(div2((tid+1))), div2(div2(div2((tid+1)))), other_tid, (other_tid+1), div2((other_tid+1)), div2(div2((other_tid+1))), div2(div2(div2((other_tid+1))))
...
#define upsweep_instantiation \
  x2t(tid), (x2t(tid)+1), div2((x2t(tid)+1)), div2(div2((x2t(tid)+1))), div2(div2(div2((x2t(tid)+1)))), x2t(other_tid), (x2t(other_tid)+1), div2((x2t(other_tid)+1)), div2(div2((x2t(other_tid)+1))), div2(div2(div2((x2t(other_tid)+1))))
