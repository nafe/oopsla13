...
// ---------------------------------------------------------------------------
#if N != 16
  #error This specification is only valid when N=16
#endif
...
#define iselement(x)  ((0 < x) & (x < 16))
#define isthreadid(t) ((0 < t) & (t < 8))
...
#define upsweep_core(offset,result,len,x) \
  result[x] == raddf(raddf(raddf(raddf(__ite_rtype((offset >= 16) & isvertex(x,16), result[x-8], ridentity),__ite_rtype((offset >= 8) & isvertex(x,8), result[x-4], ridentity)),__ite_rtype((offset >= 4) & isvertex(x,4), result[x-2], ridentity)),__ite_rtype((offset >= 2) & isvertex(x,2), result[x-1], ridentity)),len[x])
...
#define upsweep_barrier(tid,offset,result,len) \
  (__implies((tid < 8) & (offset >= 1), upsweep(offset,result,len,ai_idx(1,tid))) & \
  __implies((tid < 4) & (offset >= 4), upsweep(offset,result,len,ai_idx(2,tid))) & \
  __implies((tid < 2) & (offset >= 8), upsweep(offset,result,len,ai_idx(4,tid))) & \
  __implies((tid < 1) & (offset >= 16), upsweep(offset,result,len,ai_idx(8,tid))) & \
  __implies((tid < 8) & (offset <= 2), upsweep(offset,result,len,bi_idx(1,tid))) & \
  __implies((tid < 4) & (offset == 4), upsweep(offset,result,len,bi_idx(2,tid))) & \
  __implies((tid < 2) & (offset == 8), upsweep(offset,result,len,bi_idx(4,tid))) & \
  __implies((tid < 1) & (offset == 16), upsweep(offset,result,len,bi_idx(8,tid))))

#define upsweep_d_offset \
  ((d == 8 & offset == 1) | (d == 4 & offset == 2) | (d == 2 & offset == 4) | (d == 1 & offset == 8) | (d == 0 & offset == 16))

#define upsweep_permissions(offset,result,len,x) \
  {ghostread2 = ghostread4 = ghostread8 = ghostread16 = false; \
...
#define upsweep_barrier_permissions(tid,offset,result,len) \
  {bool ghostread2, ghostread4, ghostread8, ghostread16; \
  if ((tid < 8) && (offset >= 1)) upsweep_permissions(offset,result,len,ai_idx(1,tid)) \
  if ((tid < 4) && (offset >= 4)) upsweep_permissions(offset,result,len,ai_idx(2,tid)) \
  if ((tid < 2) && (offset >= 8)) upsweep_permissions(offset,result,len,ai_idx(4,tid)) \
  if ((tid < 1) && (offset >= 16)) upsweep_permissions(offset,result,len,ai_idx(8,tid)) \
  if ((tid < 8) && (offset <= 2)) upsweep_permissions(offset,result,len,bi_idx(1,tid)) \
  if ((tid < 4) && (offset == 4)) upsweep_permissions(offset,result,len,bi_idx(2,tid)) \
  if ((tid < 2) && (offset == 8)) upsweep_permissions(offset,result,len,bi_idx(4,tid)) \
  if ((tid < 1) && (offset == 16)) upsweep_permissions(offset,result,len,bi_idx(8,tid))}
...
#define downsweep_core(offset,result,ghostsum,x) \
  (result[x] == __ite_rtype(isvertex(x,mul2(offset)), raddf(raddf(raddf(__ite_rtype((offset <= 4), term(ghostsum,3,x), ridentity),__ite_rtype((offset <= 2), term(ghostsum,2,x), ridentity)),__ite_rtype((offset <= 1), term(ghostsum,1,x), ridentity)),__ite_rtype((offset <= 0), term(ghostsum,0,x), ridentity)), ghostsum[x]))

#if defined(FORCE_NOOVFL) || (defined(INC_ENDSPEC) && defined(BINOP_ADD))
#define downsweep_nooverflow(offset,result,ghostsum,x) \
  (__implies(isvertex(x,mul2(offset)), __add_noovfl(__ite_rtype((offset <= 4), term(ghostsum,3,x), ridentity), __ite_rtype((offset <= 2), term(ghostsum,2,x), ridentity), __ite_rtype((offset <= 1), term(ghostsum,1,x), ridentity), __ite_rtype((offset <= 0), term(ghostsum,0,x), ridentity))))
...
#define downsweep_barrier(tid,offset,result,ghostsum) \
  (__implies((tid < 1) & (offset >= 4), downsweep(offset,result,ghostsum,ai_idx(8,tid))) & \
  __implies((tid < 2) & (offset >= 2), downsweep(offset,result,ghostsum,ai_idx(4,tid))) & \
  __implies((tid < 4) & (offset >= 1), downsweep(offset,result,ghostsum,ai_idx(2,tid))) & \
  __implies((tid < 8) & (offset >= 0), downsweep(offset,result,ghostsum,ai_idx(1,tid))) & \
  __implies((tid < 1) & (offset >= 4), downsweep(offset,result,ghostsum,bi_idx(8,tid))) & \
  __implies((tid < 2) & (offset == 2), downsweep(offset,result,ghostsum,bi_idx(4,tid))) & \
  __implies((tid < 4) & (offset == 1), downsweep(offset,result,ghostsum,bi_idx(2,tid))) & \
  __implies((tid < 8) & (offset == 0), downsweep(offset,result,ghostsum,bi_idx(1,tid))))

#define downsweep_d_offset \
  ((d == 1 & offset == 16) | (d == 2 & offset == 8) | (d == 4 & offset == 4) | (d == 8 & offset == 2) | (d == 16 & offset == 1))
...
#define downsweep_barrier_permissions(tid,offset,result,ghostsum) \
  {if ((tid < 1) && (offset >= 4)) downsweep_permissions(offset,result,ghostsum,ai_idx(8,tid)) \
  if ((tid < 2) && (offset >= 2)) downsweep_permissions(offset,result,ghostsum,ai_idx(4,tid)) \
  if ((tid < 4) && (offset >= 1)) downsweep_permissions(offset,result,ghostsum,ai_idx(2,tid)) \
  if ((tid < 8) && (offset >= 0)) downsweep_permissions(offset,result,ghostsum,ai_idx(1,tid)) \
  if ((tid < 1) && (offset >= 4)) downsweep_permissions(offset,result,ghostsum,bi_idx(8,tid)) \
  if ((tid < 2) && (offset == 2)) downsweep_permissions(offset,result,ghostsum,bi_idx(4,tid)) \
  if ((tid < 4) && (offset == 1)) downsweep_permissions(offset,result,ghostsum,bi_idx(2,tid)) \
  if ((tid < 8) && (offset == 0)) downsweep_permissions(offset,result,ghostsum,bi_idx(1,tid))}
...
#define final_upsweep_barrier(tid,result,len) \
  (__implies((tid < 8), upsweep(/*offset=*/N,result,len,ai_idx(1,tid))) & \
  __implies((tid < 4), upsweep(/*offset=*/N,result,len,ai_idx(2,tid))) & \
  __implies((tid < 2), upsweep(/*offset=*/N,result,len,ai_idx(4,tid))) & \
  __implies((tid < 1), upsweep(/*offset=*/N,result,len,ai_idx(8,tid))) & \
  __implies((tid < 1), upsweep(/*offset=*/N,result,len,bi_idx(8,tid))))

#define final_downsweep_barrier(tid,result,len) \
  (__implies((tid < 8), downsweep(/*offset=*/0,result,ghostsum,ai_idx(1,tid))) & \
  __implies((tid < 8), downsweep(/*offset=*/0,result,ghostsum,bi_idx(1,tid))))

#if defined(SPEC_THREADWISE)
#define upsweep_instantiation \
  tid, div2(tid), div2(div2(tid)), other_tid
#elif defined(SPEC_ELEMENTWISE)
#define upsweep_instantiation \
  x2t(tid), x2t(div2(tid)), x2t(div2(div2(tid))), x2t(other_tid)
#endif
