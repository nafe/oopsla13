...
#define upsweep_core(offset,result,len,x) \
  result[x] == raddf(raddf(__ite_rtype((offset >= 4) & isvertex(x,4), result[x-2], ridentity),__ite_rtype((offset >= 2) & isvertex(x,2), result[x-1], ridentity)),len[x])
...
#define downsweep_core(offset,result,ghostsum,x) \
  (result[x] == __ite_rtype(isvertex(x,mul2(offset)), raddf(__ite_rtype((offset <= 1), term(ghostsum,1,x), ridentity),__ite_rtype((offset <= 0), term(ghostsum,0,x), ridentity)), ghostsum[x]))

#if defined(FORCE_NOOVFL) || (defined(INC_ENDSPEC) && defined(BINOP_ADD))
#define downsweep_nooverflow(offset,result,ghostsum,x) \
  (__implies(isvertex(x,mul2(offset)), __add_noovfl(__ite_rtype((offset <= 1), term(ghostsum,1,x), ridentity), __ite_rtype((offset <= 0), term(ghostsum,0,x), ridentity))))
...
#define downsweep_barrier_permissions(tid,offset,result,ghostsum) \
  {if ((tid < 1) && (offset >= 1)) downsweep_permissions(offset,result,ghostsum,ai_idx(2,tid)) \
  if ((tid < 2) && (offset >= 0)) downsweep_permissions(offset,result,ghostsum,ai_idx(1,tid)) \
  if ((tid < 1) && (offset >= 1)) downsweep_permissions(offset,result,ghostsum,bi_idx(2,tid)) \
  if ((tid < 2) && (offset == 0)) downsweep_permissions(offset,result,ghostsum,bi_idx(1,tid))}
